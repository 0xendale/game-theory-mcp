...
//! mapping comes from `game_theory_core::plan_to_strategy_index`, the same enumeration
...
/// taken from the game as `game-theory-core` built it, so nothing is recomputed here.
...
///
/// Each plan is placed at the index `game_theory_core::plan_to_strategy_index` assigns
...
fn plans_of(tree: &ValidExtensiveGame, strategic: &ValidStrategicGame) -> Vec<WirePlayerPlans> {
    ...
    for player in 0..tree.n_players() {
        ...
        for plan in every_plan(tree, &nodes) {
            ...
            let strategy = game_theory_core::plan_to_strategy_index(tree, player, &raw);
...
mod tests {
    ...
    fn the_second_movers_four_strategies_are_spelled_out_as_plans() {
        ...
        // Mixed-radix with the LAST node varying fastest, matching game-theory-core's
